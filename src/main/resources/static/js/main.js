...
const fun = () => {
	...
	// we will use ajaz to send request to server
	$.ajax({
		url: '/createOrder',
		data: JSON.stringify({
			amount: amount,
...
// now calling functio of updatePaymentOnServer
function updatePaymentOnServer(payment_id, order_id, status1)
{
	$.ajax({
		url: '/update_donation',
		data: JSON.stringify({
		...
		error:function(error){
			console.log("unable to send request to server")
		}
	})
}